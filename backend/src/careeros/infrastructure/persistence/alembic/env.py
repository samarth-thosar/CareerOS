...
Reads the database URL from the same layered Settings the app uses at runtime, so migrations and the
application never disagree about which database they're pointed at.
"""
...
def _run_migrations_sync(connection: AsyncConnection) -> None:
