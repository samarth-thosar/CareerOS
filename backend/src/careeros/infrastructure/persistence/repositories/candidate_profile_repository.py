...
CandidateProfile is a single-row aggregate; `get()` returns that row once it has been seeded.
"""
...
def _to_domain(model: CandidateProfileModel) -> CandidateProfile:
    ...
    return CandidateProfile(
        ...
        role_interests=[
            RoleInterest(title=role["title"], priority=role["priority"]) for role in model.role_interests
        ],
...
class SqlAlchemyCandidateProfileRepository:
    ...
    async def save(self, profile: CandidateProfile) -> None:
        ...
        model.role_interests = [
            {"title": role.title, "priority": role.priority} for role in profile.role_interests
        ]
        ...
        model.preferred_locations = list(preferences.preferred_locations) if preferences else []
